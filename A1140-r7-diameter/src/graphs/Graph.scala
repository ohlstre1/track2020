...
class Graph(val nofVertices: Int, edges: Seq[(Int,Int)]) {
  ...
  def eccentricity(v: Int): Int = {
    ...
    val listOfVisited = scala.collection.mutable.Set[Int]()
    var count = 0 ;var location = v; var i = 0
    
    def inner( listOfVisited :scala.collection.mutable.Set[Int]):Int ={
      if(listOfVisited.size < nofVertices){
         var newNeighbour = (neighbours(location) -- listOfVisited).toSet
      }
      0
    }
    
    while(listOfVisited.size < nofVertices){
      var newNeighbour = (neighbours(location) -- listOfVisited).toSet
      if(!newNeighbour.isEmpty)
      newNeighbour.foreach(listOfVisited.add(_))
      count += 1
