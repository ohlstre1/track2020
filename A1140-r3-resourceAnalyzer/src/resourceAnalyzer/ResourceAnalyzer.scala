...
class ResourceAnalyzerFast extends ResourceAnalyzer {
  ...
  protected var aliveTasksEndTime = scala.collection.mutable.PriorityQueue[Long]().reverse
  protected var minResources_ = 0
  ...
  def newTask(task: Task) = {
    ...
    while(!aliveTasksEndTime.isEmpty && prevStart >= aliveTasksEndTime.head) aliveTasksEndTime.dequeue() 
    //aliveTasksEndTime.filter( _> task.start)
    
    aliveTasksEndTime.enqueue(task.end) 
    minResources_ = minResources_ max aliveTasksEndTime.length
  }
  def minResources: Int = { minResources_
  }
}
